...
type NewLabelProps = {
  newLabelOpened: boolean;
  setNewLabelOpened: (newLabelOpened: boolean) => void;
};
...
const NewLabel = ({ newLabelOpened, setNewLabelOpened }: NewLabelProps) => {
  ...
  const onCreateLabel = async () => {
    ...
    if (res.data) {
      ...
      showNotification({
        autoClose: 5000,
        title: "Label created",
        message: res.data.createLabel.name,
        color: "blue",
        icon: <Check size={18} />,
      });
    }
  ...
  return (
    ...
    >
      ...
      >
        <Button compact variant="default" onClick={() => setNewLabelOpened(false)}>
          Cancel
        </Button>
        <Button
          compact
